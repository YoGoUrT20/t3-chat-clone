...
import { PanelLeft, Search, LogIn } from 'lucide-react';
import { useAuth } from '../AuthContext';
import { useNavigate } from 'react-router-dom';

function ChatPanel({ onReset }) {
  const { user, loading } = useAuth();
  const navigate = useNavigate();

  return (
    <aside className="left-panel-bg p-4 space-y-6 flex flex-col" style={{ width: 258, background: 'linear-gradient(to top, #0F0A0D 0%, #1B1219 100%)' }}>
      <div className="relative flex items-center justify-center h-10">
        <button className="absolute left-0 p-1" style={{ transition: 'background 0.2s' }} onMouseOver={e => e.currentTarget.style.background = '#261922'} onMouseOut={e => e.currentTarget.style.background = ''}>
          <PanelLeft size={16} color="#E3BAD1" />
        </button>
        <a className="relative flex h-8 w-24 items-center justify-center text-sm font-semibold text-foreground mx-auto" href="/" data-discover="true">
      ...
      <div className="flex items-center pb-2 border-b" style={{ borderColor: '#322028', borderBottomWidth: '1px' }}>
      ...
      <div>
        {loading ? null : user ? (
          <button
            ...
            className="flex items-center space-x-2 p-2 rounded-lg w-full group"
          ...
          <button
            ...
            className="flex items-center space-x-2 p-2 rounded-lg w-full group"
            style={{ background: 'transparent', border: 'none', outline: 'none', cursor: 'pointer', transition: 'background 0.2s', position: 'relative', top: '-5px' }}
            onMouseOver={e => e.currentTarget.style.background = '#261922'}
            onMouseOut={e => e.currentTarget.style.background = 'transparent'}
          >
