...
function getShortcutsFromStorageOrUser(user) {
  ...
  if (user && Array.isArray(user.shortcuts)) {
    currentShortcuts = user.shortcuts
  } else if (Array.isArray(userObj.shortcuts)) {
    currentShortcuts = userObj.shortcuts
  }
  
  if (currentShortcuts.length > 0) {
    const descriptions = new Set(currentShortcuts.map(s => s.description))
    const missingShortcuts = defaultShortcuts.filter(ds => !descriptions.has(ds.description))
    if (missingShortcuts.length > 0) {
      const updatedShortcuts = [...currentShortcuts, ...missingShortcuts]
      const finalShortcuts = defaultShortcuts.map(ds => updatedShortcuts.find(us => us.description === ds.description))
    ...
    return (
      <div className={`${isMobile ? 'relative w-full h-auto flex flex-col items-center justify-center p-2' : 'absolute inset-0 w-full h-full flex flex-col items-center justify-center'} transition-opacity duration-400 opacity-100 pointer-events-auto`} style={isMobile ? {} : {zIndex:1}}>
        <div className='flex flex-col items-center w-full animate-pulse'>
          <div className='w-28 h-28 rounded-full bg-[#232228] mb-3' />
          <div className='h-5 w-24 bg-[#232228] rounded mb-2' />
          <div className='flex flex-col items-center gap-2 mt-2 w-full'>
            <div className='h-7 w-full bg-[#232228] rounded mb-1' />
            <div className='h-7 w-full bg-[#232228] rounded mb-1' />
            <div className='h-7 w-full bg-[#232228] rounded' />
            <div className='w-full flex flex-col items-center mt-2'>
              <div className='w-full h-2.5 rounded-full bg-[#232228] mb-1' />
              <div className='flex justify-between w-full mt-1 text-xs'>
                <div className='h-4 w-16 bg-[#232228] rounded' />
                <div className='h-4 w-20 bg-[#232228] rounded ml-2' />
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className={`${isMobile ? 'relative w-full h-auto flex flex-col items-center justify-center p-2' : 'absolute inset-0 w-full h-full flex flex-col items-center justify-center'} transition-opacity duration-400 opacity-100 pointer-events-auto`} style={isMobile ? {} : {zIndex:2}}>
      <img
        ...
        className='w-28 h-28 rounded-full border border-[#3B3337] mb-3 object-cover'
        onError={e => { e.currentTarget.onerror = null; e.currentTarget.src = '/default-avatar.png'; }}
      />
      <span className='text-base text-center text-white font-semibold max-w-[140px] truncate'>{user.displayName}</span>
      <div className='flex flex-col items-center gap-2 mt-2 w-full'>
        <button
          className='flex items-center gap-2 px-3 py-1 rounded-md bg-[#232228] text-xs text-white hover:bg-[#28262b] transition w-full justify-center'
          onClick={() => {
        ...
        <Button
          variant='ghost'
          className='flex items-center gap-2 w-full justify-center text-[#ff6b81] mt-1'
          type='button'
      ...
      {isMobile ? null : (
        <div className='w-full mt-6 flex flex-col gap-3 rounded-2xl border border-[#232228] bg-gradient-to-br from-[#232228] to-[#18171a] p-5 shadow-sm'>
          <div className='flex items-center gap-2 mb-2'>
            <span className='text-base font-bold text-white tracking-tight'>Shortcuts</span>
          </div>
          <div className='flex flex-col gap-2'>
            {shortcuts.map((sc, i) => (
              <div key={i} className='flex flex-row items-center justify-between gap-4 group whitespace-nowrap'>
                <div className='flex flex-row items-center gap-2 flex-shrink-0'>
                  {sc.keys.map((k, idx) => (
                    <span key={idx} className='px-3 py-1 rounded-lg bg-[#232228] border border-[#a97ca5] font-mono text-base font-bold text-white transition-all group-hover:scale-105 group-hover:bg-[#2a1e2b]'>
                      {k.toLowerCase() === 'meta' || k.toLowerCase() === 'ctrl' ? (isMac() ? '⌘' : 'CTRL') : k.toUpperCase()}
                    </span>
                  ))}
                </div>
                <span
                  className='font-semibold text-[#e3cdde] text-sm text-right truncate max-w-[120px] relative flex items-center gap-1'
            ...
            <Button
              variant='outline'
              className='mt-2 w-full flex items-center justify-center gap-2 text-[#e3cdde]'
              type='button'
